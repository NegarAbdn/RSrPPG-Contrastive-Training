...
parser.add_argument('-uc0t', '--use_crf0_tmaps', type=int, default=False, help='Use crf0 tmaps')
args = parser.parse_args()
use_crf_0 = bool(args.use_crf0_tmaps)
datasets = args.data.split(",")  # MULTIPLE DATASETS
if args.fold != "whole":
...
for ds in datasets:
    tr, val = create_datasets(ds, fold, train_stride=576, seq_len=576, train_temp_aug=False)
    train_dirs.extend(tr)
    valid_dirs.extend(val)

# === Step 2: Apply exclusions AFTER creating train_dirs ===
def exclude_video07(data_dirs):
    return [
        entry for entry in data_dirs
        if not os.path.basename(entry[0]).endswith("_07")
    ]

def exclude_persons(data_dirs, person_ids_to_exclude):
    return [
        entry for entry in data_dirs
        if not any(os.path.basename(entry[0]).startswith(f"{pid:02d}_") for pid in person_ids_to_exclude)
    ]

persons_to_exclude = [15]  # or [13, 15, 16]
train_dirs = exclude_video07(train_dirs)
train_dirs = exclude_persons(train_dirs, persons_to_exclude)
valid_dirs = exclude_video07(valid_dirs)
valid_dirs = exclude_persons(valid_dirs, persons_to_exclude)

dataset = "_".join(datasets)  # For naming output folder

name_of_run_base = args.name+"_singlet_hrfft_"+dataset+args.fold+"_"+str(args.margin).replace(".","_")+"_b"+str(args.batch)+"_pre"+str(args.pre)+"_e"+str(args.epochs)
run_dir = os.path.join("output_trained", name_of_run_base)
os.makedirs(run_dir, exist_ok=True)
name_of_run = os.path.join(run_dir, name_of_run_base)
...
train_dataset = mst(data=train_dirs,stride=train_stride,shuffle=False, Training = True, transform=transforms,seq_len=seq_len,load_crf_0_tmap=use_crf_0)
valid_dataset = mst(data=valid_dirs,stride=seq_len,shuffle=False, Training = False, transform=transforms,seq_len=seq_len,load_crf_0_tmap=use_crf_0)
...
new_train_dataset = mst(data=new_train_dirs,stride=train_stride,shuffle=False, Training = True, transform=transforms,seq_len=seq_len,load_crf_0_tmap=use_crf_0)
train_loader = DataLoader(new_train_dataset,batch_size=BATCH_SIZE,num_workers=NUM_WORKERS,pin_memory=True,drop_last=True)

#if args.pre == 1:
    #model_path = "trained/mstmap2tmap_"+dataset+args.fold+".pt"
    #if not os.path.exists(model_path):
        #raise Exception(f"Could not find pre-trained model at {model_path}")
    #model.load_state_dict(torch.load(model_path))
    #model.load_state_dict(torch.load("trained/mstmap2tmap_" + dataset + args.fold + "_againn.pt"))  # negar added
if args.pre == 1:
    if use_crf_0:
        model_path = "trained/mstmap2tmap_" + dataset + args.fold + "crf0tmaps.pt"
    else:
        model_path = "trained/mstmap2tmap_" + dataset + args.fold + ".pt"
