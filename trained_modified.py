...
def validate(valid_loader, model, epoch,name_of_run):
    ...
    for i, (patched_map,tmap,masked_map,bvpmap,gt_hr,fps,wave,idx) in enumerate(valid_loader):
         # Skip batch if any video file in the batch ends with '_07'_negae
        #if any(os.path.basename(w).endswith("_07") for w in wave):
         #  continue
        data_time.update(time.time() - end)
        patched_map = patched_map.to(device=device, dtype=torch.float)
...
# Remove video type 07 (e.g., '10_07') and specific persons
...
if args.pre == 1:
    if use_crf_0:
        ...
        #model_path = "trained/mstmap2tmap_"+dataset+args.fold+"_no15"".pt"
